...
// This template matches a dyn_var argument and pushes a new argument in the function
// declaration
...
struct extract_signature<ClassType, RetType, dyn_var<T>, FutureArgTypes...> {
	...
	struct from {
			...
			static std::function<void(void)> call(builder_context *context, int arg_count, ClassType func, RestArgTypes&...rest_args, OtherArgs...other_args) {
				...
				return extract_signature<ClassType, RetType, FutureArgTypes...>::template from<RestArgTypes..., dyn_var<T>&>::template with<OtherArgs...>::call(context, arg_count + 1, func, rest_args..., *new_arg, other_args...);
			}
		};
	};
};

// This template specialization matches a non dyn_var argument and simply forwards values
// from other_args
template <typename ClassType, typename RetType, typename T, typename...FutureArgTypes>
...
struct extract_signature<ClassType, RetType> {
	...
	struct from {
                template <typename...OtherArgs>
                struct with {
                        // At this point rest of the OtherArgs will just be thrown away
