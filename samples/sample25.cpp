...
static dyn_var<int> foo(dyn_var<int> x) { return x + 1; }
...
static dyn_var<int> foobar(dyn_var<int> x, int &iter,
			   dyn_var<void(int)> &print_val) {
	dyn_var<int> t = 0;
...
int main(int argc, char *argv[]) {
	...
	auto ast = context.extract_function_ast(
	    [=](dyn_var<int> x, dyn_var<int> y,
		dyn_var<int> w) -> dyn_var<int> {
		    dyn_var<int> z = x - y - w;
		    return 0;
	    },
	    "func1");
